...
import (
	...
	"github.com/momentodevs/Momento/database"
...
var (
	...
	commands = []*discordgo.ApplicationCommand{
		...
		{
			Name:        "shuffle",
			Description: "Shuffles the queue (keeps currently playing song)",
		},
		{
			Name:        "nowplaying",
			Description: "Shows the currently playing song with queue status",
		},
		{
			Name:        "webui",
	...
	commandHandlers = map[string]func(s *discordgo.Session, i *discordgo.InteractionCreate){
		...
		"nowplaying": func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			if manager.FindUserVoiceState(s, i.GuildID, i.Member.User.ID) != nil {
				if server[i.GuildID].IsPlaying() {
					el := server[i.GuildID].Queue.GetFirstElement()
					progress := manager.FormatDuration(float64(server[i.GuildID].Frames.Load()) / constants.FrameSeconds)
					queueLength := server[i.GuildID].Queue.GetLength()

					embedMsg := embed.NewEmbed().SetTitle(s.State.User.Username).
						SetColor(0x7289DA).
						SetThumbnail(el.Thumbnail).
						AddField(constants.NowPlayingTitle,
							fmt.Sprintf("[%s](%s)\n%s / %s\nAdded by: %s\nPosition: %d of %d",
								el.Title, el.Link, progress, el.Duration, el.User))

					if queueLength > 1 {
						allQueue := server[i.GuildID].Queue.GetAllQueue()
						nextSongs := ""
						for j := 1; j < 3 && j < queueLength; j++ {
							nextSongs += fmt.Sprintf("%d. [%s](%s) - %s added by %s\n", j, allQueue[j].Title, allQueue[j].Link, allQueue[j].Duration, allQueue[j].User)
						}
						if queueLength > 4 {
							nextSongs += fmt.Sprintf("... and %d more", queueLength-4)
						}
						embedMsg.AddField("Next Up", nextSongs)
					}

					embed.SendAndDeleteEmbedInteraction(s, embedMsg.MessageEmbed, i.Interaction, time.Second*15, nil)
				} else {
					embed.SendAndDeleteEmbedInteraction(s,
						embed.NewEmbed().SetTitle(s.State.User.Username).
							AddField(constants.ErrorTitle, constants.NotPlayingSong).
							SetColor(0x7289DA).MessageEmbed,
						i.Interaction, time.Second*5, nil)
				}
			} else {
				embed.SendAndDeleteEmbedInteraction(s,
					embed.NewEmbed().SetTitle(s.State.User.Username).
						AddField(constants.ErrorTitle, constants.NotInVC).
						SetColor(0x7289DA).MessageEmbed,
					i.Interaction, time.Second*5, nil)
			}
		},
		...
		"shuffle": func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			if manager.FindUserVoiceState(s, i.GuildID, i.Member.User.ID) != nil {
				if server[i.GuildID].DjModeCheck(s, i, owners, nil) {
					return
				}

				if server[i.GuildID].IsPlaying() {
					if server[i.GuildID].Queue.GetLength() >= 2 {
						server[i.GuildID].ShuffleQueue()
						embed.SendAndDeleteEmbedInteraction(s,
							embed.NewEmbed().SetTitle(s.State.User.Username).
								AddField(constants.ShuffleTitle, constants.QueueShuffled).
								SetColor(0x7289DA).MessageEmbed,
							i.Interaction, time.Second*5, nil)
					} else {
						embed.SendAndDeleteEmbedInteraction(s,
							embed.NewEmbed().SetTitle(s.State.User.Username).
								AddField(constants.ErrorTitle, constants.TooFewToShuffle).
								SetColor(0x7289DA).MessageEmbed,
							i.Interaction, time.Second*5, nil)
					}
				} else {
					embed.SendAndDeleteEmbedInteraction(s,
						embed.NewEmbed().SetTitle(s.State.User.Username).
							AddField(constants.QueueTitle, constants.QueueEmpty).
							SetColor(0x7289DA).MessageEmbed,
						i.Interaction, time.Second*5, nil)
				}
			} else {
