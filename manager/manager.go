...
// ShuffleQueue shuffles the current queue (keeping now playing at the top)
func (server *Server) ShuffleQueue() {
	if !server.IsPlaying() || server.Queue.GetLength() < 2 {
		return
	}

	server.Queue.Shuffle()

	go notify(notification.NotificationMessage{Notification: notification.Shuffle, Guild: server.GuildID})
}
