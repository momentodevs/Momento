...
// Shuffle randomizes the queue (keeps currently playing song at the top)
func (q *Queue) Shuffle() {
	...
	shuffledRemaining := make([]Element, len(remaining))
	copy(shuffledRemaining, remaining)

	rand.Shuffle(len(shuffledRemaining), func(i, j int) {
		shuffledRemaining[i], shuffledRemaining[j] = shuffledRemaining[j], shuffledRemaining[i]
	})

	q.Queue = append([]Element{first}, shuffledRemaining...)
}
